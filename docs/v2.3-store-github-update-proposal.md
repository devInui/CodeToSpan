...
What's new in v2.3:
- Refreshed the popup and Options page.
- Moved Options and Check Domain under Show More.
- Added reload-required notifications for already open tabs.
- Added an R badge on the extension icon.
...
- Improved Exclude Domains controls.
- Reduced unnecessary reload warnings on excluded pages.
...
v2.3 の主な変更:
- popup と Options ページの見た目と操作性を改善しました。
- Options と Check Domain を Show More の中に整理しました。
- すでに開いているタブに再読み込みが必要な場合の通知を追加しました。
- 拡張機能アイコンに R バッジを表示するようにしました。
- popup から現在タブの hostname を Exclude Domains に追加できるようにしました。
- RUN / STOP 変更後に自動で再読み込みする設定を追加しました。
- Exclude Domains の操作性を改善しました。
